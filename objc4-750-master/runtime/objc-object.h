...
/// 是否为class
/// objc_class继承自objc_object, 类与元类都是用的objc_class
inline bool
objc_object::isClass()
{
    // 1.tag pointer
    if (isTaggedPointer()) return false;
    // 2.object的isa是class, class的isa是metaClass
...
/// 是否为tag pointer, iOS优化小型类型string、date... 不用指针，而用常量，避免引用计数
inline bool 
...
/// 初始化isa, alloc的时候调用(no nonpointer, no hasCxxDtor)
...
/// 修改isa
inline Class objc_object::changeIsa(Class newCls) {
...
inline void objc_object::clearDeallocating() {
    // sidetable_clearDeallocating与clearDeallocating_slow, 都是走的weak_clear_no_lock来清理
    
    // 判断该对象的引用计数是否过大
    if (slowpath(!isa.nonpointer)) {
        // 清空弱引用 Slow path for raw pointer isa.
        sidetable_clearDeallocating();
    }
    // weakly_referenced代表对象被指向或者曾经指向一个 ARC 的弱变量。has_sidetable_rc判断该对象的引用计数是否过大
...
inline void objc_object::rootDealloc() {
    ...
    {
        // 当前对象没有实例变量，则直接将当前对象free
        assert(!sidetable_present());
        free(this);
    } 
    else {
        // 如果是arc环境，并且当前定义了实例变量
...
ALWAYS_INLINE id objc_object::rootRetain(bool tryRetain, bool handleOverflow) {
    ...
    do {
        transcribeToSideTable = false;
        // 2.获取isa, 并设置oldisa、newisa
        oldisa = LoadExclusive(&isa.bits);
        newisa = oldisa;
        // 3.newisa.nonpointer = 1, 用引用表的数据 + 1
        if (slowpath(!newisa.nonpointer)) {
            ClearExclusive(&isa.bits);
...
ALWAYS_INLINE bool objc_object::rootRelease(bool performDealloc, bool handleUnderflow) {
    // 1.是tagged pointer直接返回；简单的基本数据，比如1，@"a", @"1"
    if (isTaggedPointer()) return false;
...
inline id objc_object::rootAutorelease() {
    // 1.tag pointer不管
    if (isTaggedPointer()) return (id)this;
    // 2.优化, 如果后面是strong就不注册到autorelease
