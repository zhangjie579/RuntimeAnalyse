...
#define SIDE_TABLE_WEAKLY_REFERENCED (1UL<<0)  // 弱引用
#define SIDE_TABLE_DEALLOCATING      (1UL<<1)  // MSB-ward of weak bit 正在dealloc
#define SIDE_TABLE_RC_ONE            (1UL<<2)  // MSB-ward of deallocating bit 1
#define SIDE_TABLE_RC_PINNED         (1UL<<(WORD_BITS-1)) // 运行状态
...
static id storeWeak(id *location, objc_object *newObj) {
 ...
 retry:
    ...
    // 6.有old，remove 旧的 Clean up old value, if any.
    if (haveOld) {
        // 注销
        weak_unregister_no_lock(&oldTable->weak_table, oldObj, location);
    }

    // Assign new value, if any. 7.有新的
    if (haveNew) {
        // 添加到weak表中
...
#pragma mark - AutoreleasePoolPage 自动引用释放池
/// 双向链表结构
...
#   define POOL_BOUNDARY nil
    ...
    // 1个parent, 1个child说明是双向列表结构
    // const说明指向的地址不能改, 不过指向地址的值可以改
    AutoreleasePoolPage * const parent;
    AutoreleasePoolPage *child;
    // 深度, self = parent + 1 = child - 1
    uint32_t const depth;
    // 跟parent的一样, parent没有就为0
    uint32_t hiwat;
    ...
    id *autoreleaseFullPage(id obj, AutoreleasePoolPage *page)
    {
        ...
        do {
            // 找出page的child，没有就新建 AutoreleasePoolPage
            if (page->child) page = page->child;
            else page = new AutoreleasePoolPage(page);
        } while (page->full());

        // 将此page(满的)设置为hotPage
        setHotPage(page);
        return page->add(obj);
    }
...
bool objc_object::sidetable_tryRetain() {
    ...
    bool result = true;
    RefcountMap::iterator it = table.refcnts.find(this);
    if (it == table.refcnts.end()) { // 没找到
        table.refcnts[this] = SIDE_TABLE_RC_ONE;
    } else if (it->second & SIDE_TABLE_DEALLOCATING) { // 正在dealloc
        result = false;
    } else if (! (it->second & SIDE_TABLE_RC_PINNED)) { // 找到 + 1
...
objc_object::sidetable_isDeallocating()
{
    ...
    RefcountMap::iterator it = table.refcnts.find(this);
    // != end说明有值
    // & SIDE_TABLE_DEALLOCATING 说明正在deallocating
...
uintptr_t objc_object::sidetable_release(bool performDealloc) {
    ...
    // 2.iterator遍历
    RefcountMap::iterator it = table.refcnts.find(this);
    if (it == table.refcnts.end()) { // 2.1.没找到
        do_dealloc = true;
        table.refcnts[this] = SIDE_TABLE_DEALLOCATING;
    } else if (it->second < SIDE_TABLE_DEALLOCATING) { // 2.2.正在dealloc
        // SIDE_TABLE_WEAKLY_REFERENCED may be set. Don't change it.
        do_dealloc = true;
        it->second |= SIDE_TABLE_DEALLOCATING;
    } else if (! (it->second & SIDE_TABLE_RC_PINNED)) { // 2.3. - 1
        it->second -= SIDE_TABLE_RC_ONE;
    }
    table.unlock();
...
void objc_object::sidetable_clearDeallocating() {
    ...
    // 3.不等于end, 说明有值
    if (it != table.refcnts.end()) {
        // 4.iterator first: key, second: value
        // 说明值是弱引用
...
// 一种是通过calloc开辟内存，然后通过initInstanceIsa函数初始化这块内存。第二种是直接调用class_createInstance函数，由内部实现初始化逻辑
static ALWAYS_INLINE id callAlloc(Class cls, bool checkNil, bool allocWithZone=false) {
    if (slowpath(checkNil && !cls)) return nil;

#if __OBJC2__
    // 当前的class或者是superclass是否有默认的alloc/allocWithZone:
    if (fastpath(!cls->ISA()->hasCustomAWZ())) {
        // No alloc/allocWithZone implementation. Go straight to the allocator.
        // fixme store hasCustomAWZ in the non-meta class and 
        // add it to canAllocFast's summary
        if (fastpath(cls->canAllocFast())) { // 为false
            // 当前的class或者是superclass是否有dtor初始化方法
            bool dtor = cls->hasCxxDtor();
            id obj = (id)calloc(1, cls->bits.fastInstanceSize());
            if (slowpath(!obj)) return callBadAllocHandler(cls); // 返回错误
...
/// object_getClass取的是isa(meta class)
/// 无论是instance func还是class func，全是通过isa(meta class)来判断的
+ (BOOL)isMemberOfClass:(Class)cls {
    return object_getClass((id)self) == cls;
}

- (BOOL)isMemberOfClass:(Class)cls {
    // [self class] 本质是 object_getClass(self)
