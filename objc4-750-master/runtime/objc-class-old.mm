...
/* These next three functions are the heart of ObjC method lookup. 从method list内找method
...
/// 从class内找method，只从当前class，不会到super
...
static inline old_method * _findMethodInClass(Class cls, SEL sel) {
    ...
    if (!cls->methodLists) {
    ...
    else if (cls->info & CLS_NO_METHOD_ARRAY) { // 1个method list
        // One method list.
        ...
        *mlistp = fixupSelectorsInMethodList(cls, *mlistp);
        ...
        // Multiple method lists. 多个方法list
        old_method_list **mlistp;
...
// called by a debugging check in _objc_insertMethods 从class内找selector的imp
                       ...
                       bool initialize, bool cache, bool resolver)
{
    ...
    // 1.从缓存 Optimistic cache lookup
    if (cache) {
        methodPC = _cache_getImp(cls, sel);
        if (methodPC) return methodPC;    
    }

    // 2.cls释放了 Check for freed class
    if (cls == _class_getFreedObjectClass())
        return (IMP) _freedHandler;

    // 3.判断cls是否初始化 Check for +initialize
 ...
 retry:
    ...
    // 4.cache中找到imp，就调到done去执行 Try this class's cache.
    ...
    // 5.从class中找method，只从当前class无super
    ...
    // 6.从superclass查找cache和method list Try superclass caches and method lists.
    ...
    while ((curClass = curClass->superclass)) {
        ...
        // 到这来说明cache内没找到，再找method list Superclass method list.
    ...
    // 7.到这说明自己的和super的cache和method list内都没找到 No implementation found. Try method resolver once.

    if (resolver  &&  !triedResolver) {
        methodListLock.unlock();
        // 执行消息机制的resolveMethod方法, 有class方法，有instance方法
        _class_resolveMethod(cls, sel, inst);
        triedResolver = YES;
        goto retry;
    }

    // 8.缓存method的imp为objc_forward
...
/// 找method的imp，如果为objc_forward把它设为nil
/***********************************************************************
* lookUpImpOrNil.
...
IMP lookupMethodInClassAndLoadCache(Class cls, SEL sel)
{
    ...
    if (meth) {
        ...
        // 设置method的imp为objc_msgForward Miss in method list. Cache objc_msgForward.
        _cache_addForwardEntry(cls, sel);
