...
* Locking: runtimeLock must be held by the caller.
**********************************************************************/
/// 将category_t添加到list中，并通过NXMapInsert更新所属类的category列表
                                          ...
                                          header_info *catHeader)
{
    ...
    category_list *list;
    // 获取到buckets中的value，并向value对应的数组中添加category
...
/***********************************************************************
* remethodizeClass 把class内没注册的category注册
...
* Locking: runtimeLock must be read- or write-locked by the caller.
**********************************************************************/
/// 通过name -> protocol, 找到就返回，没找到就返回nil
static Protocol *getProtocol(const char *name)
{
    runtimeLock.assertLocked();

    // Try name as-is. 从哈希表找
...
**********************************************************************/
// 初始化传入的protocol，如果哈希表中已经存在初始化的protocol，则不做任何处理
static void
             ...
             bool headerIsPreoptimized, bool headerIsBundle)
{
    ...
    if (oldproto) {
        ...
        // 如果protocol为nil，则对其进行简单的初始化，并将protocol的isa设置为protocol类
...
/*
 ...
 4.修复函数指针l遗留
 5.将所有Protocol都添加到protocol_map中
 6.对所有Protocol做重映射
...
# endif
    ...
    // Fix up @selector references 将所有sel注册到哈希表
...
static void schedule_class_load(Class cls)
{
    ...
    // 确保super已经被添加到load列表中
    // Ensure superclass-first ordering❗️❗️❗️
    schedule_class_load(cls->superclass);

    // 将imp和class添加到调用列表
    add_class_to_loadable_list(cls);
    // 设置flags，表示已经添加class到调用列表
...
* Locking: acquires runtimeLock
**********************************************************************/
/// 创建protocol，如果已经存在，就return nil
/// 没注册的isa为___IncompleteProtocol，注册的为_Protocol
Protocol *
...
void objc_registerProtocol(Protocol *proto_gen) 
{
    ...
    // 如果protocol的isa不是___IncompleteProtocol，说明有问题
    if (proto->ISA() != oldcls) {
...
protocol_addProtocol(Protocol *proto_gen, Protocol *addition_gen) 
{
    ...
    extern objc_class OBJC_CLASS_$___IncompleteProtocol;
    Class cls = (Class)&OBJC_CLASS_$___IncompleteProtocol; // 这个是未注册的isa
    ...
    // 需要proto_gen的isa为未注册，addition_gen已经注册
...
/// protocol add property，需要在protocol register之前
void 
                     ...
                     BOOL isRequiredProperty, BOOL isInstanceProperty)
{
    ...
    // 1.进if说明是已经register的protocol，不能add property
    if (proto->ISA() != cls) {
...
/*
 replace
 class_addMethod: false
 class_replaceMethod: true
 */
static IMP 
addMethod(Class cls, SEL name, IMP imp, const char *types, bool replace)
{
    ...
    if ((m = getMethodNoSuper_nolock(cls, name))) {
        // already exists
        if (!replace) { // class_addMethod直接返回
            result = m->imp;
        } else { // 更新imp
            result = _method_setImplementation(cls, m, imp);
        }
    } else { // add 方法
        // fixme optimize, 创建1个method_list_t
              ...
              uint8_t alignment, const char *type)
{
    ...
    // No class variables, 为meta class
    if (cls->isMetaClass()) {
        return NO;
    }

    // 只有in-construction class能添加ivars
...
verifySuperclass(Class superclass, bool rootOK)
{
    ...
    // Superclass must be realized.实现
                             ...
                             size_t extraBytes)
{
    ...
    // Fail if the superclass isn't kosher.
    // getClass: 判断类名是否已用
    // verifySuperclass: superclass是否合适
    if (getClass(name)  ||  !verifySuperclass(superclass, true/*rootOK*/)) {
        return nil;
    }
    ...
    // fixme mangle the name if it looks swift-y? 初始化cls的属性
    objc_initializeClassPair_internal(superclass, name, cls, meta);
...
/// 注册class
/*
 无论是注册class还是meta class，内部都是通过NXMapInsert函数实现的
 在runtime中，所有类都是存在一个哈希表中的，在table的buckets中存储
 每次新创建类之后，都需要把该类加入到哈希表中
 */
...
id _class_createInstanceFromZone(Class cls, size_t extraBytes, void *zone, 
    ...
    // 1.当前class或superclass有ctor、dtor初始化方法
    bool hasCxxCtor = cls->hasCxxCtor();
    bool hasCxxDtor = cls->hasCxxDtor();
    bool fast = cls->canAllocNonpointer();

    // 2.会通过 alignedInstanceSize 函数获取对象原始大小( < 16 ? 16 : 得出的值, 并且是16的倍数, 是最大对象内存的整数倍)
    //  class_ro_t 中有定义 instanceSize 
...
**********************************************************************/
/* dealloc的核心实现
 1.对当前对象进行析构，会调用析构函数 .cxx_destruct,在函数内部还会进行对应的release
 2.移除当前对象的所有关联对象
 3.进行最后的clear
 */
