...
union isa_t {
    ...
    Class cls;
    // 获取object的isa: (Class)(bits & ISA_MASK), 不是直接用bits获取而是再 & ISA_MASK
    // 初始化的时候为ISA_MAGIC_VALUE
    uintptr_t bits;
    ...
    /*
    struct {
        uintptr_t nonpointer        // 是32位还是62位. 0.代表普通的指针，存储中class，meta-class对象的内存地址, 1.代表优化过，使用位域存储更多的信息
        uintptr_t has_assoc         // 是否有关联类型
        uintptr_t has_cxx_dtor      // 是否有c++析构函数或oc的析构函数
        uintptr_t shiftcls          // 存储着class、meta-class对象的内存地址信息
        uintptr_t magic             // 对象是否初始化完成
        uintptr_t weakly_referenced // 是否被弱引用或曾经被弱引用
        uintptr_t deallocating      // 是否被释放中
...
public:
    ...
    // 析构函数object may have -.cxx_destruct implementation?
    bool hasCxxDtor();
